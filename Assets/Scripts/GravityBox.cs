...
public class GravityBox : GravitySource
{
	...
	public override Vector3 GetGravity(Vector3 position)
	{
		...
		{
			strength.x = boundaryDistance.x - position.x;
			outside += 1;
		}
		...
		{
			strength.y = boundaryDistance.y - position.y;
			outside += 1;
		}
		...
		{
			strength.z = boundaryDistance.z - position.z;
			outside += 1;
		}
		...
		if(outside > 0)
		{
			float distance = (outside == 1) ? Mathf.Abs(strength.x + strength.y + strength.z) : strength.magnitude;
			if(distance > outerFalloffDistance)
			{
				return Vector3.zero;
			}

			float multiplier = gravity / distance;
			if(distance > outerDistance)
			{
				multiplier *= 1f - (distance - outerDistance) * outerFalloffFactor;
			}

			return transform.TransformDirection(strength * multiplier);
		}
		...
		if(distances.x < distances.y)
		{
			...
			{
				strength.x = GetGravityComponent(position.x, distances.x);
			}
			else
			{
				strength.z = GetGravityComponent(position.z, distances.z);
			}
		}
		else if(distances.y < distances.z)
		{
			strength.y = GetGravityComponent(position.y, distances.y);
		}
		else
		{
			strength.z = GetGravityComponent(position.z, distances.z);
		...
		return transform.TransformDirection(strength);
	}

	private float GetGravityComponent(float component, float distance)
	{
		...
		{
			strength *= 1f / (distance - innerDistance) * innerFalloffFactor;
		}

		return component > 0f ? -strength : strength;
	}
