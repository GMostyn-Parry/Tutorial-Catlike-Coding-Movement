...
public class StableFloatingRigidbody : MonoBehaviour
{
	...
	private void EvaluateSubmergence()
	{
		...
		for(int i = 0; i < buoyancyOffsets.Length; i++)
		{
			Vector3 origin = offset + transform.TransformPoint(buoyancyOffsets[i]);
			if(Physics.Raycast(origin, down, out RaycastHit hit, submergenceRange + 1f, waterMask, QueryTriggerInteraction.Collide))
			{
				submergence[i] = 1f - hit.distance / submergenceRange;
			}
			else if(!safeFloating || Physics.CheckSphere(origin, 0.01f, waterMask, QueryTriggerInteraction.Collide))
			{
				submergence[i] = 1f;
			}
		}
