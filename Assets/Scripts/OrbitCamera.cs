...
public class OrbitCamera : MonoBehaviour
{
	...
	private void UpdateFocusPoint()
	{
		...
		if(focusRadius > 0f)
		{
			...
			float t = 1f;
			if(distance > 0.01f && focusCentring > 0f)
			{
				t = Mathf.Pow(1f - focusCentring, Time.unscaledDeltaTime);
			}
	...
	bool ManualRotation()
	{
		Vector2 input = new Vector2(Input.GetAxis("Camera Vertical"), Input.GetAxis("Camera Horizontal"));

		const float epsilon = 0.001f;
		if(input.x < -epsilon || input.x > epsilon || input.y < -epsilon || input.y > epsilon)
		{
	...
	private bool AutomaticRotation()
	{
		...
		if(movementDeltaSquared < 0.000001f)
		{
			return false;
		}
	...
	private void ConstrainAngles()
	{
		...
		orbitAngles.y = orbitAngles.y % 360f;
	}
